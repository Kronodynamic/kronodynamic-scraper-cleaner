...
(defmethod {dump-output-file Parser}
  (lambda (self db)
    (let ((it (open-iterator db))
          (successful (lambda (v) (not (= (char->integer (iterator-valid v)) 0)))))
      (with-output-to-file 
        (list path: (@ self output-file) append: #t) 
        (lambda ()
          (iterator-first it)
          (let loop ((valid (successful it)))
            (if valid 
